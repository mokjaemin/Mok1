...
public class StoreControllerTest {
	...
	@Test
	@DisplayName("가게 등록 실패 : StoreDTO 잘못된 입력")
	@WithMockUser
	...
	void getStoreListSuccess() throws Exception {
		// given
		HashMap<String, Short> response = new HashMap<>();
		response.put("store1", (short) 1);
		response.put("store2", (short) 2);
		...
		ResultActions resultActions = mockMvc.perform(MockMvcRequestBuilders.get("/store/list").with(csrf())
				.contentType(MediaType.APPLICATION_JSON).param("country", "Country").param("city", "City")
				.param("dong", "Dong").param("type", "KOREAN").param("page", "1").param("size", "10"));
	...
	@Test
	@DisplayName("가게 리스트 출력 실패 : 필수 파라미터 생략")
	@WithMockUser
	void getStoreListFail() throws Exception {

		// when
		ResultActions resultActions = mockMvc.perform(MockMvcRequestBuilders.get("/store/list").with(csrf())
				.contentType(MediaType.APPLICATION_JSON).param("city", "City").param("dong", "Dong")
				.param("type", "Type").param("page", "1").param("size", "10"));
	...
	void loginStoreSuccess() throws Exception {
		...
		doReturn(response).when(storeService).loginStore(anyShort(), anyString());
	...
	@Test
	@DisplayName("가게 로그인 실패 : 필수 파라미터 생략")
	@WithMockUser
	void loginStoreFailByInput() throws Exception {

		// when
		ResultActions resultActions = mockMvc
				.perform(MockMvcRequestBuilders.get("/store").with(csrf()).contentType(MediaType.APPLICATION_JSON));
