...
public class StoreInfoDAOImpl implements StoreInfoDAO {
  ...
  public List<String> getDayOff(short storeId) {
    List<String> resultList =
        queryFactory.selectDistinct(storeRestDaysMapEntity.date).from(storeRestDaysMapEntity)
            ...
            .orderBy(storeRestDaysMapEntity.date.asc()).fetch();
    // 정보 없음
  ...
  public String deleteDayOff(short storeId) {
    List<StoreRestDaysEntity> entities = queryFactory.select(storeRestDaysEntity)
        .from(storeRestDaysEntity).leftJoin(storeRestDaysEntity.childSet, storeRestDaysMapEntity)
        .fetchJoin().where(storeRestDaysEntity.storeId.eq(storeId)).fetch();
    for (StoreRestDaysEntity entity : entities) {
      for (StoreRestDaysMapEntity map : entity.getChildSet()) {
        queryFactory.delete(storeRestDaysMapEntity)
            .where(storeRestDaysMapEntity.dayId.eq(map.getDayId())).execute();
      }
      queryFactory.delete(storeRestDaysEntity)
          .where(storeRestDaysEntity.daysId.eq(entity.getDaysId())).execute();
    }
    return "success";
  }
  ...
  public String modTableInfo(StoreTableInfoEntity newData) {
    StoreTableInfoEntity originalTable =
